...
Isolation is a `WHERE` clause on the annotation author. A grant stores `default` (the reviewer and the author; grants written as `own` before the split read the same), `all`, or `custom:` followed by user ids. A reviewer's own annotations are always included. The grant also keeps the last `custom:` list in `customScope` while it sits on `default`, so switching back restores the selection. The author sees the union. Replying to, resolving or moving an annotation is refused when the viewer cannot see it; reviewers may only resolve their own threads (and only while unreplied) or move their own annotations, and only the author accepts. Which of the visible people a viewer wants on screen is a separate, per-browser choice (the **Annotations** menu; by default everyone for the author, only themselves and the author for a reviewer), kept in `localStorage`. Hiding a person (the viewer included) is rendered with CSS on the always-fully-hydrated editor, never by leaving their annotations out of it, so toggling visibility never rebuilds the editor or drops a suggestion made this visit; hiding the viewer's own suggestions makes insertions disappear and un-strikes deletions, and hides their own comment highlights too. **Suggesting (clean)** is a separate view mode that reuses the same CSS mechanism to make only the viewer's own insertions and deletions look like plain, untracked edits (comments are unaffected); it still records ordinary suggestion annotations underneath. When a reviewer hides their own annotations while also in Suggesting (clean), hiding wins. Reviewer colour is a user field, stable across documents. **A notification is subject to the same predicate as a read**: a person is only told that someone acted when the actor's annotations are ones they could have opened anyway, so a `default`-scope reviewer is never notified of another reviewer's reply in a shared thread.
