...
When you (or obsidian-git) push to the bare repo, `post-receive` POSTs to `GIT_SYNC_URL` with `GIT_SYNC_SECRET`. That process pulls, treats changed `.md` files as new base versions, rebases annotations, and broadcasts SSE. Ingest from git does not commit back to git, so the hook cannot loop.

The hook is best-effort (it must not fail the push). If curl cannot reach Vite (`host.docker.internal:5173`) or the Compose app, the push still succeeds and the working clone stays behind until the next ingest. Opening a document or Admin also `git fetch`es the bind-mounted origin and merges if the clone is behind, so a missed notify still updates after a refresh. Hook failures are appended to `DATA_DIR/git/hook.log`.
